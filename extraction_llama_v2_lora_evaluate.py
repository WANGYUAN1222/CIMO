"""
evaluate_cimo_val.py
====================================================================
CIMO LoRA 在测试集(= 训练时的 val_ds 对应论文集)上的评测
...
★ 用法 ★
  python evaluate_cimo_val.py
  python evaluate_cimo_val.py --limit 20    # 调试,只跑前 20 篇
  python evaluate_cimo_val.py --resume      # 断点续传

★ 输出 ★
  {OUTPUT_DIR}/eval_predictions.jsonl   每篇论文的预测(便于复查)
  {OUTPUT_DIR}/eval_metrics.json        指标汇总
"""
...
LORA_PATH       = f"{UTD24_DIR}/lora_ckpt/llama3.3-70b-cimo-lora-bf16-v1"
...
FIELDS_FOR_MATCH     = ["context", "intervention", "outcome"]  # 实例匹配只看 CIO,不算 mechanism
...
def extract_json_from_text(text: str) -> Optional[Dict]:
    """从模型输出抠出最外层 JSON,容错 markdown 代码块 / 前后多余文字"""
    if not text:
        return None
    text = text.strip()
...
def aggregate_metrics(per_paper: List[Dict], threshold: float) -> Dict:
    ...
    # 字段级:把所有匹配上的 pair 的 token_f1 / rouge_l 收集起来求 micro 均值
...
def main():
    ...
    print("=" * 78)
    print("CIMO LoRA Evaluation on val_ds (held-out test)")
